...
#PK if (!(Get-Module -Name VMware.VimAutomation.Core -ErrorAction SilentlyContinue)) {

#PK  if (Test-Path -Path 'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\VMware, Inc.\VMware vSphere PowerCLI' ) {

#PK    $Regkey = 'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\VMware, Inc.\VMware vSphere PowerCLI'

#PK  }

 #PK else {

   #PK $Regkey = 'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\VMware, Inc.\VMware vSphere PowerCLI'

#PK  }

  #PK .(join-path -path (Get-ItemProperty  $Regkey).InstallPath -childpath 'Scripts\Initialize-PowerCLIEnvironment.ps1')

#PK }

#PK if (!(Get-Module -Name VMware.VimAutomation.Core -ErrorAction SilentlyContinue)) {

  #PK Write-Host "VMware modules not loaded/unable to load"
...
#PK Import-Module -Name '.\PowerNSX.psm1' -ErrorAction SilentlyContinue -DisableNameChecking
...
#PK $p = [Environment]::GetEnvironmentVariable("PSModulePath");

#PK $p += ";C:\Program Files (x86)\VMware\Infrastructure\vSphere PowerCLI\Modules\";

#PK [Environment]::SetEnvironmentVariable("PSModulePath",$p);
...
$SettingsExcel = "c:\VMWARE-Downloads\install-nsx-info-homelab.xlsx"
...
$defaultNSXConnection = Connect-VIServer -Server $NSX_VC_IP -User $NSX_VC_Username -Password $NSX_VC_Password
$connection = Connect-NSXServer -NsxServer $NSX_MGR_IP -Username admin -Password default
...

{
...

  {
    ...
    #PK if(!(New-NSXManager -NsxManagerOVF $NSXManagerOVF -Name $NSX_MGR_Name -ClusterName $NSX_VC_Cluster -ManagementPortGroupName $NSX_VC_Network -DatastoreName $NSX_VC_Datastore -FolderName $NSX_VC_Folder -CliPassword $NSX_MGR_CLI_Pass -CliEnablePassword $NSX_MGR_CLI_Pass -Hostname $NSX_MGR_Hostname -IpAddress $NSX_MGR_IP -Netmask $NSX_MGR_Netmask -Gateway $NSX_MGR_Gateway -DnsServer $NSX_MGR_DNSServer -DnsDomain $NSX_MGR_DNSDomain -NtpServer $NSX_MGR_NTPServer -EnableSsh -StartVm)) {

      #PK   Write-Host "Unable to deploy NSX Manager OVF! $_" -ForegroundColor "red"
     ...
     #PK  Write-Host "Deployed NSX Manager!" -ForegroundColor "green"

      $disconnect = Disconnect-VIServer -Confirm:$False



    #PK   Write-Host -NoNewLine "If you have other tasks for me, wait for the NSX Management Service to come online and press any key to continue..."

   #PK    $null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown')
...

{
  ...
  # Configure vCenter connection on NSX Manager
   ...
   #PK  $vc = Set-NSXManager -vCenterServer $NSX_VC_Connect_IP -vCenterUserName $NSX_VC_Connect_Username -vCenterPassword $NSX_VC_Connect_Password

  # Configure vCenter SSO on NSX Manager

   #PK  $sso = Set-NSXManager -SsoServer $NSX_VC_Connect_IP -SsoUserName $NSX_VC_Connect_Username -SsoPassword $NSX_VC_Connect_Password -AcceptAnyThumbprint

   #PK  Write-Host "Configured vCenter and SSO on NSX Manager!" -ForegroundColor "green"
...
# t. @pandom / w. http://networkinferno.net/license-nsx-via-automation-with-powercli
  ...
  #PK if($InsertLicense.IsPresent)

  #PK {

   #PK  Write-Host "Assigning NSX license to vCenter..." -ForegroundColor "yellow"

   #PK  $ServiceInstance = Get-View ServiceInstance

   #PK  $LicenseManager = Get-View $ServiceInstance.Content.licenseManager

   #PK  $LicenseAssignmentManager = Get-View $LicenseManager.licenseAssignmentManager

   #PK  $output = $LicenseAssignmentManager.UpdateAssignedLicense("nsx-netsec", $NSX_License, $NULL)



  # Check if license has been properly set

   #PK  $CheckLicense = $LicenseAssignmentManager.QueryAssignedLicenses("nsx-netsec")

   #PK  if($CheckLicense.AssignedLicense.LicenseKey -ne $NSX_License) {

   #PK    Write-Host "Setting the NSX License failed! Error: $CheckLicense" -ForegroundColor "red"
   ...
   #PK  else {
    ...
    #PK   Write-Host "Configured NSX License on vCenter!" -ForegroundColor "green"
  ...
  #PK if($DeployControllers.IsPresent)

  #PK {

   #PK  Write-Host "Deploying NSX Controllers ($NSX_Controllers_Amount)...(this will take a while)" -ForegroundColor "yellow"



  # NSX Controllers sanity check

   #PK  if($NSX_Controllers_Amount -gt 3) {

    #PK   Write-Host "NSX Controllers cannot be more than 3!" -ForegroundColor "red"
  ...
  # Create controllers
   ...
   #PK  $cluster = Get-Cluster -Name $NSX_Controllers_Cluster

    #PK $datastore = Get-Datastore -Name $NSX_Controllers_Datastore

    #PK $portgroup = Get-VirtualPortGroup -Name $NSX_Controllers_PortGroup -Distributed

    #PK $i = 1

   #PK  While ($i -le $NSX_Controllers_Amount) {

   #PK    Write-Host "Deloying controller $i ..." -ForegroundColor "yellow"

   #PK    $controller = New-NsxController -Confirm:$False -IpPool $ippool -Cluster $cluster -Datastore $datastore -PortGroup $portgroup -Password $NSX_Controllers_Password -Wait
   ...
   #PK  Write-Host "Controllers deployed!" -ForegroundColor "green"
...

{
  ...
  $prep = Install-NsxCluster -Cluster $cluster -VxlanPrepTimeout 300 -Verbose
  ...
  #PK Write-Host "Adding a multicast IP range.." -ForegroundColor "yellow"

 #PK  $ignorethis = New-NsxMulticastRange -Name "Multicast1" -Begin $NSX_VXLAN_Multicast_Range_Begin -End $NSX_VXLAN_Multicast_Range_End
  ...
  $ippool = Get-NsxIpPool -Name $NSX_VXLAN_IP_Pool_Name 
...

{
...

  {
   ...
   # $edge = New-NsxEdge -Name $Edge_Name -Cluster (Get-Cluster -Name "VSAN-Cluster") -Datastore (Get-Datastore -Name $Edge_Datastore) -FormFactor $Edge_FormFactor -Password "C!sco123C!sco123" -Hostname $Edge_Hostname -EnableHa:$enableHA -Interface $vnic0
  $edge = New-NsxEdge -Name $Edge_Name -Cluster (Get-Cluster -Name "VSAN-Cluster") -Datastore (Get-Datastore -Name $Edge_Datastore) -FormFactor $Edge_FormFactor -Password "C!sco123C!sco123"   -Interface $vnic0
...

{
...

  {
    ...
    $dlr = New-NsxLogicalRouter -Name $DLR_Name -Cluster (Get-Cluster -Name VSAN-Cluster ) -Datastore (Get-Datastore -Name $DLR_Datastore) -Interface $vnic0 -ManagementPortGroup $mgtNic

    New-NsxLogicalRouter -Name $DLR_Name -Cluster (Get-Cluster -Name "VSAN-Cluster" ) -Datastore (Get-Datastore -Name $DLR_Datastore) -Interface $vnic0 -ManagementPortGroup $mgtNic -Verbose 
