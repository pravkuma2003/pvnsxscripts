...
$NestedESXiApplianceOVA = "C:\VMWARE-Downloads\Nested_ESXi6.0u3_Appliance_Template_v1.0.ovf"
$VCSAInstallerPath = "C:\VMWARE-Downloads\VMware-VCSA-all-6.5.0-4944578.iso"

# Nested ESXi VMs to deploy
$NestedESXiHostnameToIPs = @{
"vesxi60-1" = "172.29.1.85"
"vesxi60-2" = "172.29.1.86"
"vesxi60-3" = "172.29.1.87"
}
...
$VCSAIPAddress = "172.29.1.50"
$VCSAHostname = "vcsa" #Change to IP if you don't have valid DNS
$VCSAPrefix = "24"
$VCSASSODomainName = "homelab.local"
$VCSASSOSiteName = "virtuallyGhetto"
$VCSASSOPassword = "C!sco123"
$VCSARootPassword = "C!sco123!"
$VCSASSHEnable = "true"
...
$VMGateway = "172.29.1.1"
$VMDNS = "172.29.1.29"
$VMNTP = "172.29.1.29"
$VMPassword = "C!sco123"
$VMDomain = "homelab.local"
$VMSyslog = "172.29.1.29"
...
$verboseLogFile = "vghetto-lab-deployment.log"
...
if($deployVCSA -eq 1) {
    ...
    if($DeploymentTarget -eq "ESXI") {
      ...
      $config = (Get-Content -Raw "C:\VMWARE-Downloads\embedded_vCSA_on_ESXi_v3.json") | convertfrom-json
        ...
        #PV $output = Invoke-Expression "$($VCSAInstallerPath)\vcsa-cli-installer\win32\vcsa-deploy.exe install --no-esx-ssl-verify --accept-eula $($ENV:Temp)\jsontemplate.json" -ErrorVariable vcsaDeployOutput 2>&1
        #$output = 
       #PV Invoke-Expression "d:\vcsa-cli-installer\win32\vcsa-deploy.exe install --no-esx-ssl-verify --acknowledge-ceip --accept-eula --verify-only C:\VMWARE-Downloads\VCSADEPLOYv2.json"
         Invoke-Expression "f:\vcsa-cli-installer\win32\vcsa-deploy.exe install --no-esx-ssl-verify --accept-eula --verbose C:\VMWARE-Downloads\VCSADEPLOYv3.json"
       ...
       # $config = (Get-Content -Raw "$($VCSAInstallerPath)\vcsa-cli-installer\templates\install\embedded_vCSA_on_VC.json") | convertfrom-json
        $config = (Get-Content -Raw "d:\vcsa-cli-installer\templates\install\embedded_vCSA_on_VC.json") | convertfrom-json
       ...
       #pv $config.'target.vcsa'.appliance.'deployment.network' = $VMNetwork
        $config.'target.vcsa'.appliance.'thin.disk.mode' = $true
        $config.'target.vcsa'.appliance.'deployment.option' = $VCSADeploymentSize
        $config.'target.vcsa'.appliance.name = $VCSADisplayName
        $config.'target.vcsa'.network.'ip.family' = "ipv4"
        ...
        My-Logger "Deploying the VCSA ..."
        $output = Invoke-Expression "$($VCSAInstallerPath)\vcsa-cli-installer\win32\vcsa-deploy.exe install --no-esx-ssl-verify --accept-eula C:\VMWARE-Downloads\VCSADEPLOY.json"
        #Invoke-Expression "d:\vcsa-cli-installer\win32\vcsa-deploy.exe install install --no-esx-ssl-verify --accept-eula C:\VMWARE-Downloads\VCSADEPLOY.json"
